...
class CalibrationCertificateV4:
        ...
        *,
        ...
        bindings = {
            "risk_model_bundle_sha256": (
                self.risk_model_bundle_sha256, risk_model_bundle_sha256
            ),
            "feature_contract_sha256": (
                self.feature_contract_sha256, feature_contract_sha256
            ),
            "candidate_panel_sha256": (
                self.candidate_panel_sha256, candidate_panel_sha256_value
            ),
            "calibration_policy_sha256": (
                self.calibration_policy_sha256, calibration_policy_sha256
            ),
        }
        ...
        if self.query_scope.query_encoder_bundle_sha256 != _sha(
            query_encoder_bundle_sha256, "expected query encoder bundle SHA"
        ):
            raise ValueError("calibration query encoder bundle mismatch")
        if self.query_scope.query_quality_stratum_sha256 != _sha(
            query_quality_stratum_sha256, "expected query quality stratum SHA"
        ):
        ...
        _sha(self.calibration_data_manifest_sha256, "calibration data manifest SHA")
        ...
        if tuple(ordered_metrics) != tuple(metric_names):
            raise ValueError(
                "calibration coverage must exactly match panel metrics in order"
            )
    ...
    def identity_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "calibration_data_manifest_sha256": self.calibration_data_manifest_sha256,
            "split_manifest_sha256": self.split_manifest_sha256,
            "query_scope": self.query_scope.identity_dict(),
            "query_scope_sha256": self.query_scope.sha256,
            "metric_coverages": [row.to_dict() for row in self.metric_coverages],
        }
...
class RiskPanelIdentityV4:
    ...
    def validate(self) -> None:
        ...
        if self.conservative_parent not in self.candidates:
            raise ValueError("conservative parent is not in the candidate bank")
    ...
    def identity_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "source_pcm_sha256": self.source_pcm_sha256,
            "candidate_panel_sha256": candidate_panel_sha256(self.candidates),
            "candidates": [row.to_dict() for row in self.candidates],
            "metric_names": list(self.metric_names),
            "feature_contract_sha256": self.feature_contract_sha256,
            "risk_model_bundle": self.risk_model_bundle.identity_dict(),
            "calibration_policy_sha256": self.calibration_policy_sha256,
            "calibration_certificate": self.calibration_certificate.identity_dict(),
            "calibration_certificate_sha256": self.calibration_certificate.sha256,
            ...
            "query_encoder_bundle": self.query_encoder_bundle.identity_dict(),
    ...
    def to_v3(self) -> RiskPanelIdentityV3:
        ...
        return RiskPanelIdentityV3(
            ...
            conservative_parent_certificate=self.conservative_parent_certificate,
...
class RiskPanelV4:
    ...
    def validate(self) -> tuple[int, int, int, int]:
        ...
        if upper.ndim != 4 or upper.shape != available.shape:
            raise ValueError("upper/available must share (time,band,candidate,metric)")
        if upper.shape[2] != len(self.identity.candidates) or upper.shape[3] != len(
            self.identity.metric_names
        ):
            raise ValueError("risk panel axes differ from identity")
        if np.any(~np.isfinite(upper[available])) or np.any(upper[available] < 0):
            raise ValueError("available risks must be finite and non-negative")
    ...
    def sha256(self) -> str:
        ...
        upper = np.ascontiguousarray(
            np.where(mask, np.asarray(self.upper, dtype=np.float64), 0.0), dtype="<f8"
        )
    ...
    def to_v3(self) -> RiskPanelV3:
        self.validate()
        return RiskPanelV3(self.identity.to_v3(), self.upper, self.available)
...
class RouteDecisionV4:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if labels.ndim != 2:
            raise RiskRouterV4Error("route labels must be a two-dimensional grid")
        return {
            "schema": ROUTE_SCHEMA_V4,
            "status": self.status,
            ...
            "infeasible_cells": [list(row) for row in self.infeasible_cells],
...
def _discrete_objective(
    ...
    selected = np.take_along_axis(cost, labels[..., None], axis=-1)[..., 0]
...
def exhaustive_optimal_routes_v4(
    ...
    *,
    ...
    """Two-pass exact mirror: find the true minimum, then apply tie tolerance."""
    cost = np.asarray(cost, dtype=np.float64)
    feasible = np.asarray(feasible, dtype=bool)
    if cost.ndim != 3 or feasible.shape != cost.shape:
        raise ValueError("cost/feasible must share (time,band,candidate)")
    ...
    for assignment in product(range(candidates), repeat=cells):
        labels = np.asarray(assignment, dtype=np.int32).reshape(time, bands)
        selected_feasible = np.take_along_axis(
            feasible, labels[..., None], axis=-1
        )[..., 0]
        if np.all(selected_feasible):
            evaluated.append((_discrete_objective(labels, cost, config), labels.copy()))
    if not evaluated:
        raise RiskRouterV4Error("feasible cells produced no exhaustive route")
    true_best = min(value for value, _ in evaluated)
    routes = [
        labels for value, labels in evaluated
...
def solve_risk_route_v4(
    ...
    panel.validate()
    v3: RouteDecisionV3 = solve_risk_route_v3(panel.to_v3(), config)
