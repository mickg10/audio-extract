...
def validate_optimizer_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    Required fields are intentionally not filled from PyTorch defaults.  Those
    defaults are part of the experiment identity and must be visible in the
    resolved YAML/report.
    """
...
def optimizer_provenance(optimizer) -> dict[str, Any]:
    """Serialize the effective identity-bearing facts from a live optimizer."""
    groups = []
