...
def score_candidate(cand: np.ndarray, sr: int, *, reference: np.ndarray,
    ...
    ref_width = metrics.stereo(reference)["stereo_width"]
    acc: dict[str, list[float]] = {a: [] for a in AXES}
    for w_i, seg in enumerate(_windows_or_whole(cand, windows)):
        ref_seg = list(_windows_or_whole(reference, windows))[w_i]
        ...
        acc["stereo_deviation"].append(abs(metrics.stereo(seg)["stereo_width"] - ref_width))
        ...
        if activity is not None:
            ...
            acc["hall_damage_db"].append(0.0)
    return {a: float(np.mean(v)) if v else 0.0 for a, v in acc.items()}
...
def pareto_frontier(scored: list[Scored], axes=AXES) -> list[Scored]:
    front = []
    for s in scored:
...
def rank_by_scalarized(scored: list[Scored], weights: dict | None = None, axes=AXES) -> list[Scored]:
...
def successive_halving(scored: list[Scored], keep: int, axes=AXES) -> list[Scored]:
