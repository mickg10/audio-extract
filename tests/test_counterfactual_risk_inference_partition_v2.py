...
def held_out(name: str) -> HeldOutUnitV1:
...
def certificate(unit: HeldOutUnitV1, resolution_ms: int):
    return CellPartitionCertificate(
        ...
        exact_source_report_sha256=sha(
            f"partition-source-report-{unit.work_id}"
        ),
        verifier_commit=git("partition-verifier"),
    )


def source_lineage(unit, selected):
    return ExactSourceLineageV2(
        ...
        truth_manifest_sha256=sha(f"truth-{unit.work_id}"),
        mixture_pcm_sha256=sha(f"mixture-{unit.work_id}"),
        accompaniment_pcm_sha256=sha(f"accompaniment-{unit.work_id}"),
        vocal_pcm_sha256=sha(f"vocal-{unit.work_id}"),
...
def input_manifest(p, unit, arm_id, selected):
    ...
    return InferenceInputPartitionManifestV2(
        ...
        source_lineage=source_lineage(unit, selected),
        builder_commit=git("input-builder"),
    )


def route_cell(p, unit, arm_id, selected, *, evaluation_status="ROUTE_SAFE"):
    arm = p.d0_arm if arm_id == "D0" else p.r0_arm
    manifest = input_manifest(p, unit, arm_id, selected)
    ...
    run = FrozenArmInferenceRunV3(
        ...
        feature_contract_sha256=p.feature_contract_sha256,
        metric_contract_sha256=p.metric_contract_sha256,
        inference_input_manifest_sha256=manifest.sha256(p, unit, selected),
    ...
    row = WorkArmEvaluationV3(unit, provenance, evaluation)
    attestation = RoutePartitionAttestationV2(
...
def fixture(*, mismatch=False, catastrophic=False):
    p = preregistration(held_out("one"), held_out("two"))
    certificates = {
        (unit.sha256, resolution): certificate(unit, resolution)
        for unit in p.expected_units
        for resolution in (250, 500)
    }
    registry = CellPartitionRegistry.build(
        tuple(certificates.values()), source_commit=p.source_commit
    )
    rows = []
    attestations = []
...
def test_distinct_source_report_and_risk_evidence_validate():
    p, report, registry, attestations = fixture()
    first = attestations.attestations[0].input_manifest.source_lineage
    assert first.exact_source_report_sha256 != first.exact_evidence_sha256
...
def test_run_must_name_exact_content_bearing_input_manifest():
    p, report, registry, attestations = fixture()
    first = attestations.attestations[0]
    row = next(row for row in report.work_evaluations if first.key == (
        row.unit.sha256, row.arm_id
    ))
    selected = registry.by_key()[first.input_manifest.partition_key]
    changed = dataclasses.replace(
        first.input_manifest, builder_commit=git("other-builder")
    )
    with pytest.raises(
        InferencePartitionManifestError,
