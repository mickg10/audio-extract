...
def decisions_for(result, method):
    return [
        item for item in result["method_decisions"] if item["method"] == method
    ]
...
def test_actionable_certified_o2_selects_target_singer_gate():
    ...
    assert result["recommendation"] == (
        "TRAIN_TARGET_SINGER_FROZEN_MEMBER_GATE"
    )
    assert result["selected"]["method"] == "O2_global_medoid"
    assert result["selected"]["resolution_seconds"] == "0.5"
...
def test_no_vocal_regression_blocks_an_otherwise_large_gap():
    ...
    report["resolutions"]["0.5"]["works"]["aalto_mozart_dry"][
        "no_vocal"
    ]["O2_global_medoid"]["false_positive_energy_ratio"] = 0.02
    result = evaluate_report(report)
    assert result["decision"] == "NO_ACTIONABLE_GAP_AT_TESTED_RESOLUTIONS"
    selected_resolution = [
        item for item in decisions_for(result, "O2_global_medoid")
...
def test_bad_route_seam_is_a_valid_gate_failure_not_missing_evidence():
    report = complete_report(actionable_at="0.5")
    candidate = report["resolutions"]["0.5"]["works"]["bologna_verdi"][
        "methods"
    ]["O2_global_medoid"]
    candidate["seam_check"][
        "max_time_boundary_jump_over_p99_derivative"
    ] = 10.0
    result = evaluate_report(report)
    assert result["decision"] == "NO_ACTIONABLE_GAP_AT_TESTED_RESOLUTIONS"
    row = [
...
def test_missing_artifact_identity_fails_closed():
    report = complete_report()
    del report["resolutions"]["2.0"]["works"]["bologna_verdi"][
        "methods"
    ]["O2_global_medoid"]["artifact"]["artifact_pcm_sha256"]
    result = evaluate_report(report)
    assert result["decision"] == "INCOMPLETE_EVIDENCE"
...
def test_route_must_beat_median_not_merely_a_hypothetical_single():
    ...
    report["resolutions"]["0.5"]["works"]["bologna_verdi"]["methods"][
        ...
        "O2_global_medoid", metrics(voice=-6.0, hole=1.1), seed="8"
    )
    result = evaluate_report(report)
    assert result["decision"] == "NO_ACTIONABLE_GAP_AT_TESTED_RESOLUTIONS"
